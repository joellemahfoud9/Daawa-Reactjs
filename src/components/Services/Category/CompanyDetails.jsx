...
const CompanyDetails = () => {
  ...
  const handleReviewSubmit = () => {
    if (newReview.text.trim() && newReview.userName.trim() && newReview.rating.trim() && newReview.location.trim()) {
    ...
    <div>
      ...
      <div className="max-w-4xl mx-auto p-6 bg-white shadow-lg rounded-lg min-h-screen flex space-x-6">
        ...
        <div className="flex flex-col items-center text-center w-1/3 bg-gray-50 p-4 rounded-lg shadow-md">
          ...
          />
          <h1 className="text-3xl font-semibold text-gray-900 mb-2">{company.name}</h1>
          <p className="text-yellow-500 font-semibold mb-4">Rating: {company.rating} ⭐️</p>
          ...
          <div className="p-4 bg-gray-50 rounded-lg shadow-inner mb-4">
            ...
            {categoryName === 'Catering' && company.dishes ? (
                ...
                <ul className="space-y-2">
                  {company.dishes.map((dish, index) => (
                    <li key={index} className="flex justify-between items-center">
                      ...
                      <span className="text-gray-800 font-bold">${dish.price.toFixed(2)}</span>
                      <button
                        className="bg-slate-200 px-4 py-3 ml-2"
                      >
              ...
              <div>
                ...
                <ul className="space-y-2">
                  {company.charOptions.map((char, index) => (
                    <li key={index} className="flex justify-between items-center">
                      <span className="text-gray-700">{char.type}</span><span className="text-gray-800 font-bold">${char.price.toFixed(2)}</span>
                      <button
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="text-gray-800 font-bold">${company.price?.toFixed(2) || 'N/A'}</span>
                <button
                  className="bg-slate-200 px-4 py-3 ml-2"
                >
                  <GiShoppingBag size={24} />
              ...
              <ul className="space-y-2">
                {reviews.map((review, index) => (
                  <li key={index} className="p-2 bg-gray-100 rounded-lg shadow">
                    <p><strong>{review.userName}</strong> from {review.location}</p>
