...
function ListCategory() {
  ...
  return (
    <div className="max-w-7xl mx-auto p-6">
      <h2 className="text-3xl font-semibold text-center mb-8">{category.serviceName}</h2>

      {/* Company Cards */}
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-6">
        {category.companies.map((company) => (
          <div key={company.id} className="border border-gray-200 rounded-md p-4 text-center">
            <div className="h-48 flex items-center justify-center mb-4">
              <img src={company.imageUrl} alt={company.name} className="h-full w-full object-contain" />
            </div>
            ...
            {category.serviceName === 'Catering' && company.dishes ? (
              ...
              <p className="text-gray-600 font-medium mb-4">${company.price?.toFixed(2)}</p>
            )}

            {/* show details Button */}
            <Link to={`/category/${category.serviceName}/${company.id}`}>
            <button
              className=" px-4 py-2 mt-4 border border-black"
            >
              Show Details
            </button>
            </Link>
